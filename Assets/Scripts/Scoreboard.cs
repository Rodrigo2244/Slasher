...
public class Scoreboard : MonoBehaviour {
	...
	void OnGUI(){
		...
		for(int i = 0; i < numPlayers; i++){
			if(hasWon[i] == true)
				GUI.Label(new Rect(Screen.width/2-80, Screen.height/2+i*20-300, 250, 200), "Player " +(i+1)+" has survived.",gui);
			if(hasDied[i] == true)
				GUI.Label(new Rect(Screen.width/2-80, Screen.height/2+i*20-300, 250, 200), "Player " +(i+1)+" has died.",gui);
		}
